'''
Fall 2018
Generates depth images dataset
CNN
Graphs loss
'''
## TODO: Fix 'display' -> not ipynb, use normal plt.show
...
image_tensor = torch.FloatTensor(image).permute(
    2, 0, 1)  # PIL and torch expect difft orders
...
# class matplotlib.patches.Rectangle(xy, width, height, angle=0.0, **kwargs)[source]
    ...
    rect = patches.Rectangle((x + i*IMG_X, y), 1, 20,
                             angle=orient, fill=True, color='orange')
    ax.add_patch(rect)
...
plt.show()
plt.rcParams['figure.figsize'] = [30, 5]
xposition = np.array(range(epoch)) * (total_step)
# print(xposition)
for xc in xposition:
...
for i in range(len(num_epochs)):
    ...
    losses_list = []
    print('Training model now... with: %d num_epoch and %d learning rate' %
